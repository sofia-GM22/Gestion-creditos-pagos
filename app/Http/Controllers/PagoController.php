...
class PagoController extends Controller
{
    ...
    {
        abort_if($credito->saldo <= 0, 403, 'Este crédito ya está pagado en su totalidad.');
    ...
    public function store(PagoRequest $request, Credito $credito)
    {
        ...
        DB::transaction(function () use ($datos, $credito, &$pago) {
            ...
            $creditoBloqueado = Credito::where('id', $credito->id)->lockForUpdate()->first();
            ...
            $nuevoSaldo = round($creditoBloqueado->saldo - $datos['monto'], 2);
    ...
    public function misPagos(Request $request)
{
    ...
    $pagos = Pago::with('credito.cliente')
        ->whereHas('credito', fn ($q) => $q->where('cliente_id', $cliente->id))
        ->orderByDesc('fecha_pago')
        ->orderByDesc('id')
        ->get();
    ...
    private function autorizarAcceso(Pago $pago): void
    {
        ...
        if ($usuario->esCliente()) {
            ...
            abort_unless($cliente && $pago->credito->cliente_id === $cliente->id, 403, 'No tienes permiso para ver este comprobante.');
        }
    }
}
